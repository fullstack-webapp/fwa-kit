...
The metadata epoch fences durable authority across worker globals and reset. When a pre-epoch installation is opened by a newer kernel, the kernel may create the epoch only in the same IndexedDB transaction that proves a valid active or retained release already exists. It replaces any pre-epoch candidate journal, which cannot prove a current owner identity, with an epoch-fenced cleanup marker. A Web-Locks-capable global rechecks that marker and the referenced releases under the candidate-cache lock before deleting an unreferenced candidate cache and clearing the marker; a global without Web Locks leaves the marker durable for a later capable global and may continue offline reads. A missing database, an empty database, or metadata containing only preferences and client pins does not qualify for migration, so a restarted global cannot recreate authority after reset.
